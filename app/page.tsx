...
function ConfigurationNotice({ error = false }: { error?: boolean }) {
  return (
    <section className="public-status" aria-live="polite">
      <p className="public-kicker">{error ? "No pudimos cargar la carta" : "Carta en preparación"}</p>
      ...
      <p>
        {error
          ...
          : "La carta estará disponible cuando conectemos los datos de Revuelto."}
      </p>
    </section>
  );
}
...
export function PublicHome({ data }: { data: PublicSiteData }) {
  ...
  const heroTickerText = content.heroSubtitle || content.heroTitle;

  return (
    <ClickSpark sparkColor="#ed6d2d" sparkSize={9} sparkRadius={18} sparkCount={7} duration={360}>
      ...
      <main>
        <section id="inicio" className="public-hero">
          ...
          <div className="public-hero-art" data-reveal>
            ...
            <span className="public-hero-sticker" aria-hidden="true">
              <Image src="/brand/logos/logo-compact.svg" alt="" width={100} height={100} />
            </span>
            <Image
              src="/brand/logos/illustration-eating.svg"
              alt="Ilustración de una persona disfrutando Revuelto"
              width={580}
              height={580}
              priority
              sizes="(max-width: 719px) 82vw, 42vw"
            />
          </div>

          <div className="public-hero-loop" aria-hidden="true">
            <CurvedLoop
              marqueeText={`${heroTickerText}  ●  `}
              speed={0.65}
              curveAmount={28}
              direction="left"
              interactive={false}
              className="public-hero-loop-text"
            />
          </div>
        </section>
          ...
          <>
            {(content.brandTitle || content.brandDescription) && (
              <section className="public-brand-story public-section" data-reveal>
                <div className="public-brand-visual" aria-hidden="true">
                  <Image
                    src="/brand/logos/illustration-cooking.svg"
                    alt=""
                    width={480}
                    height={480}
                    sizes="(max-width: 719px) 52vw, 28vw"
                  />
                </div>
                <div className="public-brand-copy">
                  {content.brandTitle && <h2>{content.brandTitle}</h2>}
                  {content.brandDescription && <p>{content.brandDescription}</p>}
                </div>
                <Image
                  className="public-brand-seal"
                  src="/brand/logos/seal-smile.svg"
                  alt=""
                  width={180}
                  height={180}
                  aria-hidden="true"
              ...
              <section id={PUBLIC_SECTION_IDS.bowls} className="public-section public-menu-section">
                ...
                <div className="public-bowl-grid">
                  {data.visibleBowls.map((bowl, index) => (
                    <article className="public-bowl-card" data-reveal key={bowl.id}>
                      <div className="public-bowl-number" aria-hidden="true">
                      ...
                      <div className="public-bowl-image">
                        {bowl.imageUrl ? (
                          <Image
                            ...
                            alt={bowl.name}
                            fill
                            sizes="(max-width: 719px) 92vw, (max-width: 1099px) 48vw, 38vw"
                          />
                        ) : (
                          <div className="public-bowl-placeholder">
                            <Image src="/brand/logos/logo-egg-rev.svg" alt="" width={130} height={130} />
                      ...
                      <div className="public-bowl-body">
                        ...
                        <div className="public-size-list">
                          {bowl.sizes.map((size) => (
                            <div className="public-size-row" key={size.id}>
                              <div>
                                <span className="public-size-name">{sizeLabels[size.size] ?? size.size}</span>
                                <span>{size.ounces} oz · {size.eggQuantity} huevos</span>
                              </div>
              ...
              <section id={PUBLIC_SECTION_IDS.promotions} className="public-section public-promotions-section">
                ...
                <div className="public-promotion-list">
                  {data.visiblePromotions.map((promotion, index) => (
                    <article className="public-promotion-poster" data-reveal key={promotion.id}>
                      {promotion.imageUrl && (
                        <div className="public-promotion-image">
                          <Image
                            ...
                            alt={promotion.title}
                            fill
                            sizes="(max-width: 719px) 92vw, 48vw"
                          />
                        </div>
                      )}
                      <div className="public-promotion-copy">
                        <span className="public-promotion-index" aria-hidden="true">PROMO {index + 1}</span>
                        <h3>{promotion.title}</h3>
                        <p>{promotion.description}</p>
                        <p className="public-promotion-availability">
                          <strong>Disponible</strong>
                          <span>{formatPromotionPublicAvailability(promotion)}</span>
                        </p>
                      </div>
                    </article>
                  ))}
              ...
              <section id={PUBLIC_SECTION_IDS.branches} className="public-section public-branches-section">
                ...
                <div className="public-branch-grid">
                  {data.visibleBranches.map((branch) => (
                    <article className="public-branch-card" data-reveal key={branch.id}>
                      <div className="public-branch-heading">
                        <p className="public-kicker">Punto Revuelto</p>
                        <h3>{branch.name}</h3>
                        <p>{branch.address}<br />{branch.city}</p>
                        {branch.whatsappNumber && <p className="public-branch-phone">{branch.whatsappNumber}</p>}
                      </div>
                      <ul className="public-schedule" aria-label={`Horarios de ${branch.name}`}>
                        {branch.businessHours.map((hour) => (
                          <li className={hour.isClosed ? "is-closed" : ""} key={hour.id}>
                            <span>{dayLabels[hour.dayOfWeek]}</span>
                            <strong>{hour.isClosed ? "Cerrado" : `${hour.openingTime}—${hour.closingTime}`}</strong>
                          </li>
                        ))}
                      </ul>
                    </article>
      ...
      {whatsappUrl && (
        <a className="public-whatsapp" href={whatsappUrl} target="_blank" rel="noopener noreferrer">
          <span className="public-whatsapp-dot" aria-hidden="true">↗</span>
          <span>{content.whatsappButtonText || "WhatsApp"}</span>
        </a>
      )}
      ...
      <footer className="public-footer">
        <div className="public-footer-top">
          <a href="#inicio" aria-label="Volver al inicio">
            <Image src="/brand/logos/logo-horizontal.svg" alt="Revuelto" width={560} height={210} />
          </a>
        ...
        <div className="public-footer-bottom">
          <nav aria-label="Navegación del pie">
            ...
            {menuItems.map((item) => <a href={item.href} key={item.id}>{item.label}</a>)}
          </nav>
        ...
        <Image className="public-footer-seal" src="/brand/logos/seal-light.svg" alt="" width={260} height={260} aria-hidden="true" />
        </footer>
      </div>
    </ClickSpark>
  );
