...
| Mercado Pago | Checkout Pro TEST implementado con preference, webhook firmado y verificación server-side. Un pago aprobado real ya confirmó `Payment.get()` y evidenció que `live_mode` no identifica rígidamente las credenciales TEST modernas. |
...
`POST /api/webhooks/mercado-pago` acepta solo `payment.created` y `payment.updated`, exige `x-signature`, `x-request-id` y `data.id` coherentes y siempre consulta el pago real por SDK. Retorno y webhook usan la misma reconciliación: `MERCADO_PAGO_MODE` debe ser exactamente `TEST`, el Payment debe ser accesible con la credencial TEST configurada, su `external_reference` debe identificar un `CheckoutOrder` local, el Payment ID no puede pertenecer a otra orden y monto/ARS deben coincidir exactamente. Recién entonces se persiste el estado real del proveedor. Los duplicados no repiten efectos, un aprobado no retrocede a pendiente/rechazado y un estado desconocido se conserva de manera segura como pendiente. Los fallos transitorios responden 503 para permitir reintento; firma inválida 401, body inválido 400, payload grande 413 y éxito 200.

Mercado Pago documenta credenciales TEST automáticas para Checkout Pro, cuyo Access Token puede comenzar con `APP_USR` igual que producción, y pruebas mediante Buyer Test y tarjetas/escenarios de prueba. No documenta una señal adicional contractual en Payment o Preference que permita distinguir de forma inequívoca esas credenciales modernas. En la prueba real de Revuelto, un Payment válido creado por ese flujo TEST devolvió `live_mode=true`; por eso `live_mode` se registra como diagnóstico pero no sustituye la configuración explícita ni las correlaciones. El retorno tampoco confía en los parámetros de la back URL.
